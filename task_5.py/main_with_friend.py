...
firefly = game.Friend(
    "Firefly", "A friendly firefly, who will guide you throughout the game."
)
firefly.set_conversation("Hi! I will help you win.")
kitchen.set_character(firefly)
...
while dead == False:
    ...
    if command in ["north", "south", "east", "west"]:
        ...
        if inhabitant is not None and isinstance(inhabitant, game.Friend):
            inhabitant.notify_of_danger(next_room)
            should_proceed = input("> ")

            if should_proceed == "Y":
                current_room = next_room
            else:
                print("You decided to stay in " + current_room.name)
        else:
            current_room = next_room
    elif command == "talk":
